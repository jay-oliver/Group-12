...
{
    float measurement = 3.0543*(count_left-prev_count_left); //(C_wheel/N_Wheel)*(count_left-prev_count_left)/(interval/1000)
    return measurement;
}

float Encoder::ReadVelocityRight(void)
{
    float measurement = 3.0543*(count_right-prev_count_right); //(C_wheel/N_Wheel)*(count_right-prev_count_right)/(interval/1000)
    return measurement;
}
